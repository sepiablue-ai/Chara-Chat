...
DEFAULT_CONFIG = {
    "llm": {
        "url": "http://192.168.10.108:1234/api/v1/chat",
        "api_mode": "lmstudio_native",
...
class ChatEngine:
    ...
    def _get_clothing_tags(self, clothing_state):
        ...
        for key, value in clothing_cfg.items():
            if key in state:
                return value, ""
    ...
    def process_chat_turn_stream(self, user_input_text, enable_image=True, enable_voice=False):
        ...
        instruction = (
            ...
            f"これに対する現在の情景、キャラクターの反応、および画像生成用のタグをJSONで出力してください。"
        )
        ...
        try:
            content, result = self._call_llm_with_retry(
                ...
                partial_callback=(
                    start_image_from_partial if enable_image else None
                ),
            )
        except Exception:
